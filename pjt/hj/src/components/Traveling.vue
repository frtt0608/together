    ...
    <v-container>
      <v-layout wrap align-center>
        ...
        <v-flex wrap align-center>
          <p>
            {{Traveling.props.city}} 지역의 테마별 여행지 추천을 도와드릴까요?
            <v-btn color="warning" dark v-show="click_thema==false" @click="click_thema=true"><v-icon left>add_alert</v-icon>테마 추천</v-btn>
            <v-menu v-if="click_thema" offset-y transition="slide-x-transition" bottom right>
              <v-btn slot="activator" color="#FF9800" dark class="deep-orange">{{type}}</v-btn>
              <v-list>
                <v-list-tile v-for="(item, index) in this.types" :key="index" @click="type_click(index)">
                  <v-list-tile-title>{{ item[0]}}</v-list-tile-title>
                </v-list-tile>
              </v-list>
            </v-menu>
            
            <v-menu v-if="type !=='대분류 선택'" transition="slide-y-transition" bottom>
              <v-btn slot="activator" color="#607D8B" dark class="purple">{{thema1}}</v-btn>
              <v-list>
          ...
          <p>날씨에 따라 {{Traveling.props.city}}의 관광지를 추천해 드릴까요?
            <v-btn color="info" dark @click="informWeather(Traveling.props.city)"><v-icon left>brightness_5</v-icon>날씨</v-btn></p>

          <p>미세먼지에 따라 {{Traveling.props.city}}의 관광지를 추천해 드릴까요?
            <v-btn color="error" dark @click="informDust(Traveling.props.city)"><v-icon left>cloud</v-icon>미세먼지</v-btn></p>

          <p>{{Traveling.props.city}}의 병원 위치를 알려드릴까요?
            <v-btn color="success" dark @click="informHospital(index.props.lat, index.props.lon)"><v-icon left>add_to_queue</v-icon>병원</v-btn></p>
        </v-flex>

      </v-layout>
    </v-container>

    <h1 v-if="weather_click" class="my-5 text-xs-center">
      현재 {{Traveling.props.city}}의 날씨는 {{index.props.weather.type[0].info}}입니다
      <p v-if="weather_in">실내 관광지를 추천해드리겠습니다</p>
      <p v-if="weather_out">야외 관광지를 추천해드리겠습니다</p>
    </h1>
    
    <h1 v-if="mise_click" class="my-5 text-xs-center">
      현재 {{Traveling.props.city}}의 미세먼지는 {{this.mise_inform['pm10_s']}}, 초미세먼지는 {{this.mise_inform['pm25_s']}} 이네요!
    </h1>

    <h1 v-if="hospital_click" class="my-5 text-xs-center">
      근처의 병원을 알려드리겠습니다
    </h1>
...
export default {
  ...
  methods : {
    ...
    informWeather(city) {
      this.weather_click = true
      this.hospital_click = false
      this.mise_click = false
      this.click_thema = false
      this.load = true
      this.weather_out = false
      this.weather_in = false

      console.log(index.props.weather.type[0].info)

      var r = index.props.weather.type[0].info.indexOf("rain");
      ...
      var cl = index.props.weather.type[0].info.indexOf("cloud");
      var m = index.props.weather.type[0].info.indexOf("mist");
      var f = index.props.weather.type[0].info.indexOf("fog");
      console.log(c+" "+h+" "+cl+" "+m+" "+f);

      if(r>=0 || d>=0 || s>=0) {
        console.log("실내")
        this.weather_in = true;
      } else if(c>=0 || h>=0 || cl>=0 || m>=0 || f>=0) {
        console.log("야외")
        this.weather_out = true;
      }

      this.trip_inform = []

      var cat3_lists_out = {A01010100 : '국립공원', A01010200 :'도립공원', A01010300 : '군립공원', A01010400: '산', A01010500: '자연생태관광지', A01010600: '자연휴양림', A01010700: '수목원',
      A01010800: '폭포', A01010900: '계곡', A01011000: '약수터' ,A01011100: '해안절경', A01011200: '해수욕장', A01011300: '섬', A01011400: '항구/포구', A01011500: '어촌',
      A01011600: '등대', A01011700: '호수', A01011800: '강', A01011900: '동굴', A01020100: '희귀동.식물', A01020200: '기암괴석',A02010100: '고궁', A02010200: '성',A02010300:'문',
      A02010400: '고택', A02010500: '생가', A02010600: '민속마을',A02010700: '유적지/사적지',A02010800: '사찰',A02010900:'종교성지',A02011000:'안보관광',A02020100:'유원지',
      A02020200: '관광단지', A02020600: '테마공원', A02020700: '공원',
      ...
      var cat3_lists_in = {A02020300: '온천/욕장/스파', A02020400: '이색찜질방', A02020500: '헬스투어', A02020800: '유람선/잠수함관광',A02060100:'박물관',A02060200:'기념관',A02060300:'전시관',A02060400:'컨벤션센터',
      A02060500:'미술관/화랑',A02060700:'문화원',A02060800:'외국문화원',A02060900:'도서관',A02061000:'대형서점',A02061100:'문화전수시설',A02061200:'영화관',A02061300:'어학당',A02061400:'학교',
      A02080200:'연극',A02080300:'뮤지컬',A02080400:'오페라',A02080500:'전시회',A02080600:'박람회',A02080700:'컨벤션',A02080800:'무용',A02080900:'클래식음악회',A02081000:'대중콘서트',
      A02081100:'영화',C01170001:'맛코스',A03021000:'카지노',B02010100:'관광호텔',B02010200:'수상관광호텔',B02010300:'전통호텔',B02010400:'가족호텔',B02010500:'콘도미니엄',
      B02010600:'유스호스텔',B02010700:'펜션',B02010800:'여관',B02010900:'모텔',B02011000:'민박',B02011100:'게스트하우스',B02011200:'홈스테이',B02011300:'서비스드레지던스',B02011400:'의료관광호텔',B02011500:'소형호텔',
