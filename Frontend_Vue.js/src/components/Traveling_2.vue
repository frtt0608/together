<template>
  <div v-if="this.$session.get('lo')" class="travel">
    ...
    >{{this.$session.get('lo').nickname}}님은{{lat}} 현재 {{Traveling.props.city}}에서 여행중이시네요!</h1>
      ...
      <v-layout wrap align-center>
        ...
        <v-flex wrap align-center>
          <p>
            ...
            <v-btn color="warning" dark v-show="click_thema==false" @click="click_thema=true">
              <v-icon left>add_alert</v-icon>테마 추천
            </v-btn>
            ...
            <v-menu v-if="type !=='대분류 선택'" transition="slide-y-transition" bottom>
              ...
              <v-list>
                <v-list-tile
                  v-for="( index, item ) in this.types[thema_index][1]"
                  :key="index"
                  @click="thema1_click(item)"
                >
            ...
            <v-btn icon>
              <v-icon
                v-if="click_thema"
                @click="informLocate(Traveling.props.city,type,thema1);"
              >search</v-icon>
            </v-btn>
          </p>

          <p>
            날씨에 따라 {{Traveling.props.city}}의 관광지를 추천해 드릴까요?
            <v-btn color="info" dark @click="informWeather(Traveling.props.city)">
              <v-icon left>brightness_5</v-icon>날씨
            </v-btn>
          </p>

          <p>
            미세먼지에 따라 {{Traveling.props.city}}의 관광지를 추천해 드릴까요?
            <v-btn color="error" dark @click="informDust(Traveling.props.city)">
              <v-icon left>cloud</v-icon>미세먼지
            </v-btn>
          </p>
    ...
    informHospital(lat, lon) {
      ...
      axios.get(url).then((res) => {
        ...
        var mapContainer = document.getElementById('hospimap'), // 지도를 표시할 div
          mapOption = {
            center: new kakao.maps.LatLng(index.props.lat, index.props.lon), // 지도의 중심좌표
            level: 6 // 지도의 확대 레벨
          };
